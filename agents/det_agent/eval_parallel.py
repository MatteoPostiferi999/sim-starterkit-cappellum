...
def _post_with_retry(client, path, json_body=None, tries=4):
    """POST with retry on connection/read errors and 5xx."""
    last_err = None
    for attempt in range(tries):
        try:
            r = client.post(path, json=json_body) if json_body is not None else client.post(path)
            if r.status_code >= 500:
                last_err = httpx.HTTPStatusError(f"5xx", request=r.request, response=r)
...
def _run_one(
    ...
    started = time.time()
    try:
        with httpx.Client(base_url=base_url, timeout=90.0, transport=transport) as client:
            ...
            for turn in range(30):
                ...
                for tc in tool_calls:
                    rr = _post_with_retry(client, f"/games/{game_id}/action", tc)
                    res = rr.json()
                ...
                rr = _post_with_retry(client, f"/games/{game_id}/end-turn")
                turn_data = rr.json()

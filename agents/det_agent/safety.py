...
def validate(plan: list[dict], state) -> list[dict]:
    ...
    for act in plan:
        ...
        if tool == "place_order":
            ...
            if base <= 0:
                continue
            # Clamp — keep strictly inside the (0.80, 1.20) range so API doesn't
            # reject boundary values like base*1.20 exactly.
            lo = base * (PRICE_MULT_MIN + 0.002)
            hi = base * (PRICE_MULT_MAX - 0.002)
